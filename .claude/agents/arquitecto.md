...
description: >-
  ...
  always first — read H4 packaging yourself + mechanical / OT / OSHB skeletons), H2 developments
  named from consecutive H3s, H1 major developments named from consecutive H2s, a book's telos
  identified, or a Title/Subtitle proposed. Not for Writer `>` commentary (that is Escriba) or
  Observer clause coding.
...
- **Truncated / overlapping H4s** (`…y la`, adjacent H4s repeating ≥3 words) — **blocking
  packaging failure**, not upstream soft debt. **Read** the `####` sequence yourself (no script):
  dangling endings, seam overlaps ≥3 words, LBF verses missing from the student surface. On broken
  packaging: verdict **Bloqueado**; do not name H2/H1/telos/title.
- **Daniel lesson:** JSON row counts and “spans left as fill” are not a green light. The
  student-facing H4 surface must be clean under your own reading before naming.
...
**Blocking** — hand back before naming anything: broken H4 packaging (from your reading), any
provisional/unanswered clause, any cycle, any H4 opening with a subordinator, any uncovered verse
range, any command buried as a dependent or a phrase. **Note** — proceed and carry it into
*Dudas*: relative-of-connection roots, demotions that landed as orphans without moving a boundary,
attachment-order flags contained inside one unit.
