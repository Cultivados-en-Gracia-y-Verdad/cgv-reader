...
description: >-
  ...
  invoked, when verifying a skeleton's independent clauses (H4s) before structuring — including
  buried imperatives, false purpose H4s, wrong purpose parents, and biblical reading order —
  naming H2 developments from consecutive H3s, naming H1 major developments from consecutive
  H2s, identifying a book's telos, or proposing a book Title and Subtitle. Keeps workshop notes
  out of the student manual. Do not use for Writer `>` commentary (that is Escriba) or Observer
  structure coding.
---
